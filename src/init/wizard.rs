...
pub async fn run(target_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // LLM provider — the pulse's brain. An agent CLI is one of the installed
    ...
    // Wire the pulse for its agent CLI. Everything lands inside the pulse
    // directory — nothing in the user's home — so any number of pulses can
        ...
        crate::cli_provider::adapters::for_config(&config.provider, config.adapter.as_deref())
    {
        ...
        let mut results =
            super::agent_bootstrap::ensure_common(&pulse_dir, integration.recall_echo_provider());
        results.extend(
            integration.ensure(&pulse_dir, &super::agent_bootstrap::find_recall_echo_bin()),
        );
        for item in &results {
            println!("  {item}");
        }
    ...
    // them from the pulse home (`pulses/` / legacy `pulses/` container
