...
/// point at the entity root, the adapter's environment hygiene is applied.
    ...
    let (system_prompt_file, effective_prompt) = match adapter.system_prompt_delivery() {
        ...
        SystemPromptDelivery::Argv { max_bytes } => {
            if system_prompt.len() > max_bytes {
                return Err(CliError::UnsupportedCapability {
                    adapter: adapter.name(),
                    bin: String::new(),
                    capability: "a system prompt this large on argv",
                });
            }
...
async fn ensure_capability(adapter: &dyn CliAdapter, bin: &str) -> Result<(), CliError> {
...
/// Reduce an NDJSON body to the reply: the last `Result` line wins, and the
/// deltas are kept for the case where the CLI never wrote one.
    ...
    match terminal {
        ...
        Some((text, false, usage)) => {
            ...
            if text.trim().is_empty() {
                Err(format!("{} returned an empty result", adapter.name()))
            } else {
                Ok(Reply {
        ...
        None => Err(format!("{} stream ended without a result", adapter.name())),
    }
}
...
impl LmProvider for CliProvider {
    fn invoke(
        ...
        Box::pin(async move {
            ...
            ensure_capability(adapter, &bin).await?;
...
impl StreamingProvider for CliProvider {
    ...
    fn invoke_streaming(
        ...
        Box::pin(async_stream::stream! {
            ...
            if let Err(e) = ensure_capability(adapter, &bin).await {
            ...
            match adapter.output_mode(true) {
                OutputMode::Ndjson => {
                    ...
                    match terminal {
                        // Some CLIs report failures in the terminal record rather
                        // than by exiting non-zero mid-stream; quota exhaustion and
                        // policy refusals both arrive this way.
                        Some((text, true, _)) => {
                        ...
                        Some((text, false, usage)) => {
                            ...
                            if final_text.trim().is_empty() {
                                yield StreamEvent::Error(format!("{name} returned empty output"));
                            } else {
                        ...
                        None => {
                            yield StreamEvent::Error(format!("{name} stream ended without a result"));
                        }
                    }
                }
...
/// Expose the argv builder for the given adapter and inputs, for tests.
#[cfg(test)]
pub(crate) fn args_for(adapter: &dyn CliAdapter, inv: &Invocation<'_>) -> Vec<std::ffi::OsString> {
    adapter.invoke_args(inv)
}
